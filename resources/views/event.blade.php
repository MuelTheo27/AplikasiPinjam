        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventClick: function(info) {
                    ...
                    document.getElementById('updateSelectedDate').value = moment(info.event.start)
                        .format('DD-MM-YYYY');
                    ...
                    updateSaveBtn.onclick = function() {
                        ...
                        const [day, month, year] = document.getElementById('updateSelectedDate')
                            .value.split('-');
                        const formattedDate = `${year}-${month}-${day}`;
                        ...
                        fetch(`/calUpdate/${id}`, {
                            ...
                            .catch(async error => {
                                if (error.json) {
                                    const err = await error.json();
                                    if (err.errors && err.errors.title) {
                                        document.getElementById('updateTitleError')
                                            .innerText = err.errors.title[0];
                                    }
                                } else {
            ...
            document.getElementById('bookingModal').addEventListener('hide.bs.modal', function() {
                if (document.activeElement && document.getElementById('bookingModal').contains(document
                        .activeElement)) {
            ...
            document.getElementById('updateModal').addEventListener('hide.bs.modal', function() {
                if (document.activeElement && document.getElementById('updateModal').contains(document
                        .activeElement)) {
                    document.activeElement.blur();
                }
            });
